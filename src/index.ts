...
const sidebarApp = new Vue({
  ...
  computed: {
    searchResults(): string[] {
      return this.fuse.search(this.searchQuery).map((value: any) => value.item.objectId.toString());
    },
    fuse(): any {
  ...
  methods: {
    ...
    createFilter(filter: string, value: string): void {
    ...
    checkFilter(item: MapItem): boolean {
...
let uniforms: {
  ...
  zoomDistance: { value: number };
  scale: { value: number };
  control: { value: Vector3 };
};
...
let grid: Object3D;
const sceneLights: { sun: Object3D | null; stars: Object3D | null } = { sun: null, stars: null };
let movement: { x: number[]; y: number[]; z: number[]; zoom: number[] } = {
  x: [], y: [], z: [], zoom: [],
};
let initialised = false;
const scale: HTMLElement = document.getElementById('scale');
...
function easeBetween(
  ...
  const m: { x: number[]; y: number[]; z: number[]; zoom: number[] } = {
    x: [], y: [], z: [], zoom: [],
  };
  const d: { x: number; y: number; z: number; zoom: number } = {
    x: 0, y: 0, z: 0, zoom: 0,
  };
  ...
  for (let i = 0; i < duration; i += 1) {
    const t: number = easeInOutQuad((i + 1) / duration) - easeInOutQuad(i / duration);
    m.x.push(d.x * t);
    m.y.push(d.y * t);
    m.z.push(d.z * t);
...
function lookAt(objectId: string) {
  const object = mapData.find((item) => item.objectId === objectId);
  movement = easeBetween(
    controls.target,
    new Vector3(object.pos.x, object.pos.y, object.pos.z),
...
function openItem(objectId: string) {
  sidebarApp.searchQuery = mapData.find((val) => val.objectId === objectId).name;
...
async function init() {
  ...
  // Camera using a perspective view (as opposed to orthographic).
  // Aspect ratio set to match window and camera renders from 0.1 units away to 10^6 units away.
  renderer = new WebGLRenderer({ antialias: true }); // Three.js-libs renderer.
  ...
  uniforms.texture.value.wrapS = RepeatWrapping;
  uniforms.texture.value.wrapT = RepeatWrapping;
  ...
  for (const mapObject in mapData) {
    const {
      scale, pos, rot, modelPath,
    } = mapData[mapObject];
...
function animate() {
  ...
  while (spawnedAsteroids !== settings.asteroidCount && asteroidsToLoad === 0) {
    if (asteroids.length < settings.asteroidCount) {
      ...
      const radius: number = 14000 + Math.random() * 2200;
      const angle: number = Math.random() * 2 * Math.PI;
      lod.object.position.set(
        Math.cos(angle) * radius + (Math.random() - 0.5) * 1000 + 15000,
        (Math.random() - 0.5) * 42,
        Math.sin(angle) * radius + (Math.random() - 0.5) * 1000,
      );
      lod.object.scale.set(
        (Math.random() + 0.5) * 10,
        (Math.random() + 0.5) * 10,
        (Math.random() + 0.5) * 10,
      );
      lod.object.rotation.set(
  ...
  scale.innerText = `1 Grid Tile = ${(settings.zoomIncrement ** zoomFactor).toString()}km × ${(settings.zoomIncrement ** zoomFactor).toString()}km.`;
